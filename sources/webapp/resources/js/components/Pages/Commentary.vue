<template>
  <div class="px-4 overflow-hidden bg-white md:max-w-[1225px] md:mx-auto md:mb-auto md:px-12 lg:px-24 xl:px-32 lg:py-12">
    ...
    <div class="flex flex-col items-center my-8 space-y-6 md:my-12">
      <div v-if="loadedCommentary.original_language !== loadedCommentary.locale" class="p-4 text-sm font-medium text-white bg-ok-red">
        {{ $t("ATTENTION: This version of the commentary is an automatic machine translation of the original. The original version is in :original_language. The translation was done with www.deepl.com. Only the original version is authoritative. The translated form of the commentary cannot be cited.", { original_language: $t(loadedCommentary.original_language) }) }}
      </div>
      ...
      <div class="text-center lg:text-xl">
        <p v-if="loadedCommentary.assigned_authors && loadedCommentary.assigned_authors.length > 0 && loadedCommentary.assigned_authors[0] !== ''">
          {{ $t('commentary_by') }} <i>{{ loadedCommentary.assigned_authors.join(' ' + $t('and') + ' ') }}</i>
        </p>
        <p v-if="loadedCommentary.assigned_editors && loadedCommentary.assigned_editors.length > 0 && loadedCommentary.assigned_editors[0] !== ''">
          {{ $t('edited_by') }} <i>{{ loadedCommentary.assigned_editors.join(' ' + $t('and') + ' ') }}</i>
        </p>
      </div>
    ...
    <template v-if="loadedCommentary.pdf_commentary_filename !== ''">
      <h2 class="mt-12 mb-4 font-sans text-xl tracking-wider uppercase">
        Download PDF
      </h2>
      <p>
        <a :href="loadedCommentary.pdf_commentary_path + loadedCommentary.pdf_commentary_filename" download>
    ...
    <p>
      Onlinekommentar.ch, {{ $t('commentary_on') }} {{ loadedCommentary.title }} {{ $t('creative_commons_text') }}
    </p>
...
<script setup>
  ...
  const loadedCommentary = ref(props.commentary)
  ...
  const loadVersionWithTimestamp = (timestamp) => {
    ...
    axios.get('/' + props.locale + '/commentaries/' + props.commentary.id + '/revisions/' + timestamp)
      .then(response => {
        // load the selected revision
        loadedCommentary.value = response.data
